...
function GupPet_CanFly()
  ...
  -- Siren Isle (TWW)
	...
	if not(IsFlyableArea()) and not(IsAdvancedFlyableArea()) then
		return false
	else
		...
		-- for i=1, C_PvP.GetNumWorldPVPAreas() do
		-- 	local _, localizedName, isActive = C_PvP.GetWorldPVPAreaInfo(i)
		-- 	if localizedName == GetZoneText() and isActive then
		-- 		return false
		-- 	end
		-- end
		local isActive = C_PvP.IsActiveBattlefield()
