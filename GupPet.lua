...
-- SAVED VARS
GUPPET_SAVEDDATA = GUPPET_SAVEDDATA or { Ground = { Total=0  } , Fly = { Total=0} , Aquatic = { Total=0} , Unknown= { Total=0}, Companion = { Total=0}  }

GUPPET_SAVEDLOCATIONS = GUPPET_SAVEDLOCATIONS or { [GUPPET_C["M_GLOBALWORLD"]] = 0 , [GUPPET_C["M_CITIES"]] = 10 , [GUPPET_C["M_INSTANCES"]] = 20 , [GUPPET_C["M_ARENAS"]] =30 , [GUPPET_C["M_BATTLEGROUNDS"]] = 40  }

GUPPET_OPTIONS = GUPPET_OPTIONS or { Debug = false , Mode = "Expert_", NewVersion = 0 , PreviewFrameSpeed = 1, PreviewFrameStartPos = 0, NewLearnedOn = true,
...
-- Updated from UIMenus to UISpecialFrames for modern compatibility
tinsert(UISpecialFrames, "GupPet_InterfaceOptionsFrame")
tinsert(UISpecialFrames, "GupPet_IconPopupFrame")


local table_sort = table.sort

-- Helper to handle C_Spell return table changes in 11.x
local function GupPet_GetSpellName(spellID)
...
function GupPet_OnLoad(self)
    ...
    -- LEARNED_SPELL_IN_TAB is deprecated, using SPELLS_CHANGED
...
function GupPet_OnEvent(self, event, ...)
    ...
    elseif event == "ADDON_LOADED" then
        ...
        if arg1 == "GupPet" then
            ...
            -- Fix legacy typo: ResummonEveryWhere -> ResummonEverywhere (and keep both in sync)
            if GUPPET_OPTIONS.AutoCompanion.ResummonEverywhere == nil and GUPPET_OPTIONS.AutoCompanion.ResummonEveryWhere ~= nil then
                GUPPET_OPTIONS.AutoCompanion.ResummonEverywhere = GUPPET_OPTIONS.AutoCompanion.ResummonEveryWhere
            end
            if GUPPET_OPTIONS.AutoCompanion.ResummonEverywhere == nil then
                GUPPET_OPTIONS.AutoCompanion.ResummonEverywhere = false
            end
            if GUPPET_OPTIONS.AutoCompanion.ResummonEveryWhere == nil then
                GUPPET_OPTIONS.AutoCompanion.ResummonEveryWhere = GUPPET_OPTIONS.AutoCompanion.ResummonEverywhere
            end
            ...
            -- Don't clobber the whole SavedVariables table if a key is missing
            if not GUPPET_SAVEDDATA.Ground then GUPPET_SAVEDDATA.Ground = { Total = 0 } end
            if not GUPPET_SAVEDDATA.Multi then GUPPET_SAVEDDATA.Multi = { Total = 0 } end
            if not GUPPET_SAVEDDATA.Fly then GUPPET_SAVEDDATA.Fly = { Total = 0 } end
            if not GUPPET_SAVEDDATA.Aquatic then GUPPET_SAVEDDATA.Aquatic = { Total = 0 } end
            if not GUPPET_SAVEDDATA.Unknown then GUPPET_SAVEDDATA.Unknown = { Total = 0 } end
            if not GUPPET_SAVEDDATA.Companion then GUPPET_SAVEDDATA.Companion = { Total = 0 } end
        end
    ...
    if event == "PLAYER_REGEN_DISABLED" then
        ...
        elseif GUPPET_TEMPCLASSDATA.Type == "MAGE" then
            ...
            if GUPPET_OPTIONS["Class"]["SlowFallOnFalling"] and GUPPET_TEMPCLASSDATA["SlowFall"]["Usable"] then
                local NitroBoost = GupPet_GetSpellName(54861) or "Nitro Boosts"
                local macroText = "/use [nomounted] "..GUPPET_TEMPCLASSDATA["SlowFall"]["Name"] .."\n/script GupPet_Exit()"
                if NitroBoost then
                    macroText = "/cancelaura "..NitroBoost.."\n"..macroText
                end
                
                _G["GupPet_IngameFrameTemplateAuto"]:SetAttribute("macrotext", macroText)
                _G["GupPet_IngameFrameTemplateGround"]:SetAttribute("macrotext", macroText)
            end
...
function GupPet_IsInBook( spell )
    ...
    tempvar.Name = GupPet_GetSpellName(spell)
    tempvar.Spell = spell
    
    -- Check if the spell is known/usable by the player
...
function GupPet_UpdateClassDataList()
    ...
    if GUPPET_TEMPCLASSDATA.Type == "HUNTER" then
        ...
        GUPPET_TEMPCLASSDATA["Cheetah"] = GupPet_IsInBook( 186257 ) --GUPPET_CLASSDATA.HUNTER.Cheetah )

    elseif GUPPET_TEMPCLASSDATA.Type == "DRUID" then

        GUPPET_TEMPCLASSDATA["Travel"]  = GupPet_IsInBook( 783 ) --GUPPET_CLASSDATA.DRUID.Travel )
        GUPPET_TEMPCLASSDATA["Cat"]  	= GupPet_IsInBook( 768 ) --GUPPET_CLASSDATA.DRUID.Cat )

    elseif GUPPET_TEMPCLASSDATA.Type == "SHAMAN" then

        GUPPET_TEMPCLASSDATA["Ghost"] = GupPet_IsInBook( 2645 ) --GUPPET_CLASSDATA.SHAMAN.Ghost )
        GUPPET_TEMPCLASSDATA["WaterWalking"] = GupPet_IsInBook( 546 ) --GUPPET_CLASSDATA.SHAMAN.WaterWalking )

    elseif GUPPET_TEMPCLASSDATA.Type == "MAGE" then

        GUPPET_TEMPCLASSDATA["SlowFall"]  = GupPet_IsInBook( 130 ) --GUPPET_CLASSDATA.MAGE.SlowFall )

    elseif GUPPET_TEMPCLASSDATA.Type == "DEATHKNIGHT" then

        GUPPET_TEMPCLASSDATA["PathOfFrost"]  = GupPet_IsInBook( 3714 ) --GUPPET_CLASSDATA.DEATHKNIGHT.PathOfFrost )

    elseif GUPPET_TEMPCLASSDATA.Type == "PRIEST" then

        GUPPET_TEMPCLASSDATA["Levitate"]  = GupPet_IsInBook( 1706 ) -- GUPPET_CLASSDATA.PRIEST.Levitate )
    ...
    if GUPPET_TEMPRACEDATA.Type == "Worgen" then
        GUPPET_TEMPRACEDATA["RunningWild"] = GupPet_IsInBook( 87840 ) --GUPPET_RACEDATA.Worgen.RunningWild )
    end

    ------
    -- GUPPET_TEMPCLASSDATA["SpiritOfRedemption"] = {}
    -- GUPPET_TEMPCLASSDATA["SpiritOfRedemption"]["Name"]  = GupPet_GetSpellName( GUPPET_CLASSDATA.PRIEST.SpiritOfRedemption )
    GUPPET_TEMPCLASSDATA["Invisibility"] = {}
    GUPPET_TEMPCLASSDATA["Invisibility"]["Name"]  = GupPet_GetSpellName( 66 ) --GUPPET_CLASSDATA.MAGE.Invisibility )
    GUPPET_TEMPCLASSDATA["FeignDeath"] = {}
    GUPPET_TEMPCLASSDATA["FeignDeath"]["Name"]  = GupPet_GetSpellName( 5384 ) --GUPPET_CLASSDATA.HUNTER.FeignDeath )
    GUPPET_TEMPCLASSDATA["Camouflage"] = {}
    GUPPET_TEMPCLASSDATA["Camouflage"]["Name"]  = GupPet_GetSpellName( 199483 ) --GUPPET_CLASSDATA.HUNTER.Camouflage )
    ------
end
...
local function GupPet_ProfessionUpdate()
    local profEngineer, profTailor, profLeather = 0, 0, 0
    
    -- Updated for C_TradeSkillUI API
    local professions = C_TradeSkillUI.GetProfessions("player")
    if professions then
        for _, profID in pairs(professions) do
            local profInfo = C_TradeSkillUI.GetProfessionInfo(profID)
            if profInfo then
                if profInfo.skillLineID == 202 then profEngineer = profInfo.skillLevel
                elseif profInfo.skillLineID == 197 then profTailor = profInfo.skillLevel
                elseif profInfo.skillLineID == 165 then profLeather = profInfo.skillLevel
                end
            end
        end
    end
...
local function GupPet_IsMountUsable(mountID, creatureSpellID, isFactionSpecific, faction, hideOnChar, isCollected, isUsable, classSpecific)
    ...
    -- mount journal already accounts for riding skill, location, profession, etc.
    ...
    -- Profession gating is handled by the mount journal (isUsable), so we don't hardcode specific spellIDs here.
...
function GupPet_UpdateDataList()
    ...
    for i, mountID in ipairs(C_MountJournal.GetMountIDs()) do
        local creatureName, creatureSpellID, _, _, isUsable, _, _, isFactionSpecific, faction, hideOnChar, isCollected = C_MountJournal.GetMountInfoByID(mountID)
        ...
        if GupPet_IsMountUsable(mountID, creatureSpellID, isFactionSpecific, faction, hideOnChar, isCollected, isUsable, DBMountData and DBMountData.class) then
            if DBMountData then
            ...
            if detected == false then
                ...
                -- Fallback categorization by mountTypeID when the mount isn't in Gup_MountData.
                -- Types drift over time, so unknowns go to "Unknown" instead of disappearing.
                if mountType == 247 or mountType == 248 or mountType == 398 or mountType == 402 or mountType == 407 or mountType == 424 then

                    -- Flying-capable mounts: keep legacy behavior (show in Fly + Ground lists)
                    GUPPET_SAVEDDATA_TEMP["Fly"]["Total"] = GUPPET_SAVEDDATA_TEMP["Fly"]["Total"] + 1
                    GUPPET_SAVEDDATA_TEMP["Fly"][ GUPPET_SAVEDDATA_TEMP["Fly"]["Total"] ] =  { Name = creatureName , Id = creatureSpellID, displayID = creatureID ,Slot = mountID , Weight = GupPet_CheckWeightData( "Fly" , creatureSpellID )  }
                ...
                elseif mountType == 230 or mountType == 241 or mountType == 269 or mountType == 284 or mountType == 408 or mountType == 412 then
                ...
                elseif mountType == 231 or mountType == 232 or mountType == 254 then
                    ...
                    -- Unknown/new type: keep it visible
                    GUPPET_SAVEDDATA_TEMP["Unknown"]["Total"] = GUPPET_SAVEDDATA_TEMP["Unknown"]["Total"] + 1
                    GUPPET_SAVEDDATA_TEMP["Unknown"][ GUPPET_SAVEDDATA_TEMP["Unknown"]["Total"] ] =  { Name = creatureName , Id = creatureSpellID, displayID = creatureID ,Slot = mountID , Weight = GupPet_CheckWeightData( "Unknown" , creatureSpellID )  }
                end
